...
from duckgpt.s3.storage import duckgpt_s3_storage
from duckgpt.s3.agent import duckdb_s3_agent
from duckgpt.s3.semantic_model import get_semantic_model
...
def get_duckgpt_s3_conversation(
    ...
    return Conversation(
        ...
        1. Find which tables you need to query. Use `show_tables` to get the available tables.
        2. Find which columns you need to use. Use `describe_table` to describe each table you'd like to query.
        ...
        5. Inspect the query using `inspect_query` to confirm it is correct.
        6. Run the query using the `run_query` function
        7. Analyse the results and return the answer in markdown format.
