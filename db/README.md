...
```bash
docker exec -it junior-de alembic -c db/alembic.ini revision --autogenerate -m "Initialize DB"
```
...
```bash
docker exec -it junior-de alembic -c db/alembic.ini upgrade head
```
...
```bash
ECS_CLUSTER=junior-de-prd-cluster
TASK_ARN=$(aws ecs list-tasks --cluster junior-de-prd-cluster --query "taskArns[0]" --output text)
CONTAINER_NAME=junior-de
